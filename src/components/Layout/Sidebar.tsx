...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onNavigate, isOpen = false, onClose }) => {
  ...
  // State para conexão com o BD/Internet
  ...
  return (
    ...
    >
      {/* Header / Logo */}
      <div className="h-16 flex items-center justify-between px-6 border-b border-gray-100/10 shrink-0">
         <img 
            src={config.logoUrl} 
            alt="Logo" 
      ...
      {/* User Mini Profile */}
      <div className="px-6 py-4 mb-2 flex flex-col shrink-0">
      ...
      <div className="p-4 border-t border-gray-100/10 mt-auto shrink-0">
        <div className="flex items-center space-x-3 px-3 py-2 rounded-lg bg-black/5">
            <div className={`w-3 h-3 rounded-full flex-shrink-0 transition-all duration-500 ${isOnline ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)] animate-pulse'}`}></div>
            <div className="flex flex-col">
                <span className="text-[10px] uppercase font-bold tracking-wider opacity-60" style={{ color: config.theme.sidebarTextColor }}>Banco de Dados</span>
                <span className={`text-xs font-bold truncate ${isOnline ? 'text-green-600' : 'text-red-500'}`}>
