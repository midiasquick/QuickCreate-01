...
import { User, Role, EmailTemplate, Notice, RoleDefinition, MenuItemConfig, SmtpConfig } from '../types';
import { Trash2, Edit2, Plus, Save, X, Upload, Mail, AlertTriangle, Shield, Palette, User as UserIcon, List, Link as LinkIcon, Eye, EyeOff, Lock, CheckSquare, Server, ExternalLink, KeyRound } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 h-full overflow-hidden">
      ...
      <div className="w-full md:w-64 flex-shrink-0 space-y-2">
        <TabButton id="users" icon={UserIcon} label="Usuários" />
              ...
              <Card title="Adicionar Novo Usuário" className="mb-6 border border-primary-light">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Select label="Função" value={newUserForm.role} onChange={e => setNewUserForm({...newUserForm, role: e.target.value})}>
                    {config.roles.map(role => <option key={role.id} value={role.id}>{role.name}</option>)}
                  </Select>
                </div>
                <div className="mt-4 flex justify-end space-x-2">
                  <Button variant="secondary" onClick={() => setIsAddingUser(false)}>Cancelar</Button>
                  <Button variant="success" onClick={handleAddUser}>Salvar</Button>
                </div>
              </Card>
            )}
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-1">
                            ...
                            <button onClick={() => { if(confirm('Excluir usuário?')) deleteUser(user.id) }} className="text-red-400 hover:text-red-600 p-1" title="Excluir usuário">
                                <Trash2 size={18} />
                            </button>
                        </div>
                      </td>
